...
function KioskPairingLinkCard(): JSX.Element {
  const origin = typeof window !== 'undefined' ? window.location.origin : '';
  const url = `${origin}/kiosk/pair`;
  const [copied, setCopied] = useState(false);
  return (
    <section className="mb-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
      <h2 className="text-sm font-semibold text-slate-900">Kiosk pairing URL</h2>
      <p className="mt-1 text-xs text-slate-500">
        Open this URL on a dedicated tablet, then enter the 8-digit code generated above to pair it
        to this company.
      </p>
      <div className="mt-3 flex items-center gap-2 rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
        <p className="min-w-0 flex-1 truncate font-mono text-xs text-slate-800">{url}</p>
        <button
          type="button"
